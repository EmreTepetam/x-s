...
class DriverInstallerApp(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle(self.translations["driver_name"][self.current_language])
        ...
        self.driver_name_label = QLabel(self.get_driver_name(), self)
    ...
    def toggle_language(self):
        """
        Dil değiştirme butonuna basıldığında çalışır.
        """
    ...
    def update_language(self):
        """
        Programdaki tüm metinleri seçilen dile göre günceller.
        """
        self.setWindowTitle(self.translations["driver_name"][self.current_language])
    ...
    def start_installation(self):
        ...
        self.progress_value = 0
        self.progress_bar.setValue(0)
        self.show_feedback(self.translations["loading_message"][self.current_language], error=False)

        self.progress_bar.setValue(0)
        self.show_feedback(self.translations["loading_message"][self.current_language], error=False)
        success = self.simulate_installation()
        if success:
            self.progress_bar.setValue(100)
            self.show_feedback(self.translations["success_message"][self.current_language], error=False)
        else:
            self.progress_bar.setValue(0)
            self.show_feedback(self.translations["error_message"][self.current_language], error=True)
        ...
        try:
            ...
            if "Driver package added successfully." in stdout_output:
                if "(Already exists in the system)" in stdout_output:
                    return True, f"{os.path.basename(inf_path)} zaten sistemde mevcut."
                return True, f"{os.path.basename(inf_path)} yüklemesi başarılı."

            # Hata kontrolü
            error_message = stderr_output or "Bir hata oluştu."
            return False, f"{os.path.basename(inf_path)} yüklenemedi: {error_message}"

        except Exception as e:
            return False, f"{os.path.basename(inf_path)} yüklenemedi: Beklenmeyen bir hata oluştu: {str(e)}"
